...
class TestAutoTradeUI:
    ...
    def test_guardrail_variables_initialization(self, setup_display):
        ...
        with patch('wsapp_gui.config.app_config') as mock_app_config:
            ...
            var_max_notional = tk.DoubleVar(value=float(mock_app_config.get('autotrade.max_position_notional_per_symbol', 0.0) or 0.0))
            var_max_qty = tk.DoubleVar(value=float(mock_app_config.get('autotrade.max_position_qty_per_symbol', 0.0) or 0.0))
    ...
    def test_ledger_tree_structure(self, setup_display):
        ...
        assert first_item['values'] == ['12:34:56', 'AAPL', 'buy', 123]  # Tkinter converts to appropriate types
...
class TestConfigValidation:
    ...
    def test_config_type_coercion(self):
        ...
        try:
            ...
            notional_val = float(config.get('autotrade.max_position_notional_per_symbol', 0.0) or 0.0)
    ...
    def test_config_default_values(self):
        ...
        try:
            ...
            var_max_notional = float(config.get('autotrade.max_position_notional_per_symbol', 0.0) or 0.0)
...
def test_ui_integration_with_executor():
    ...
    with patch('wsapp_gui.config.app_config') as mock_app_config:
        ...
        var_at_max_notional = float(mock_app_config.get('autotrade.max_position_notional_per_symbol', 0.0) or 0.0)
        var_at_max_qty = float(mock_app_config.get('autotrade.max_position_qty_per_symbol', 0.0) or 0.0)
        ...
        mock_executor.configure(
            ...
            max_position_notional_per_symbol=var_at_max_notional if var_at_max_notional > 0 else None,
